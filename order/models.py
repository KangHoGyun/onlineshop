...
class OrderItem(models.Model):
    ...
    product = models.ForeignKey(Product, on_delete=models.PROTECT, related_name='order_proucts')
...
class OrderTransactionManager(models.Manager):
    def create_new(self, order, amount, success=None, transaction_status=None):
        ...
        order_hash = hashlib.shal(str(order.id).encode('utf-8')).hexdigest()
        email_hash = str(order.email).split("@")[0]
        final_hash = hashlib.shal((order_hash + email_hash).encode('utf-8')).hexdigest()[:10]
